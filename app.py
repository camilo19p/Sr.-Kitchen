...
def ce(v):
    c = {"Entregado":"#34d399", "En preparacion":"#fbbf24", "Pendiente":"#f87171", "Pagada":"#34d399",
         "Activo":"#34d399", "Inactivo":"#f87171", "Administrador":"#f87171", "Mesero":"#fbbf24"}
...
def sidebar():
    ...
    MENUS = {
        "Administrador": [("📊 Dashboard","Dashboard"), ("📦 Inventario","Inventario"), ("🛒 Pedidos","Pedidos"), ("👥 Usuarios","Usuarios"), ("🧾 Facturacion","Facturacion"), ("📈 Reportes","Reportes"), ("🤖 Prediccion","Prediccion")],
        "Mesero": [("📊 Dashboard","Dashboard"), ("🛒 Pedidos","Pedidos"), ("📋 Menu","Menu")],
        "Cliente": [("📋 Menu","Menu"), ("🛒 Pedir","MisPedidos")]
    }
    with st.sidebar:
...
def p_dashboard():
    ...
    df_p = st.session_state.productos
    df_ped = st.session_state.pedidos
    ventas = df_ped[df_ped["Fecha"] == "2024-03-10"]["Total"].sum()
    bajo = int((df_p["Stock"] <= df_p["SMin"]).sum())
    activos = len(df_ped[df_ped["Estado"] != "Entregado"])
    c1, c2, c3, c4 = st.columns(4)
    for col, title, val, sub in [
        (c1, "💰 Ventas hoy", f"${ventas:,.0f}", "COP"),
        (c2, "🛒 Pedidos activos", str(activos), "En proceso"),
        (c3, "📦 Productos", str(len(df_p)), "En inventario"),
        (c4, "⚠️ Stock bajo", f'<span style="color:#f87171">{bajo}</span>', "Reabastecer")
    ]:
    ...
    with c2:
        ...
        if bajo_df.empty:
            st.success("✅ Todo el inventario en niveles óptimos")
        else:
...
# ... (el resto de las funciones siguen exactamente igual: p_inventario, p_pedidos, p_usuarios, p_facturacion, p_reportes, p_prediccion, p_menu, p_mispedidos, main)
...
def p_inventario():
    ...
    with t1:
        ...
        b = st.text_input("🔍 Buscar", placeholder="Nombre o categoría...")
        if b:
            df = df[df["Nombre"].str.contains(b, case=False) | df["Cat"].str.contains(b, case=False)]
        def res(row):
            return ["background-color:#ef444418"] * len(row) if row["Stock"] <= row["SMin"] else [""] * len(row)
        ...
        c2.metric("📦 Total productos", len(df))
        c3.metric("⚠️ Bajo stock", len(df[df["Stock"] <= df["SMin"]]))
    with t2:
        with st.form("fp"):
            c1, c2 = st.columns(2)
            with c1:
                nombre = st.text_input("Nombre *")
                cat = st.selectbox("Categoría", ["Carnes","Verduras","Granos","Tuberculos","Aceites","Condimentos","Azucares","Lacteos","Bebidas","Otros"])
                unidad = st.selectbox("Unidad", ["kg","gr","Lt","ml","Unidad","Docena","Caja"])
            with c2:
                stock = st.number_input("Stock inicial", min_value=0, value=0)
                pc = st.number_input("Precio compra", min_value=0, value=0)
                pv = st.number_input("Precio venta", min_value=0, value=0)
                smin = st.number_input("Stock mínimo", min_value=0, value=5)
            if st.form_submit_button("✅ Registrar Producto", use_container_width=True):
                if nombre.strip():
                    nid = f"P{len(st.session_state.productos)+1:03d}"
                    new_row = pd.DataFrame([{"ID":nid,"Nombre":nombre,"Cat":cat,"Stock":stock,"Unidad":unidad,"PC":pc,"PV":pv,"SMin":smin}])
                    st.session_state.productos = pd.concat([st.session_state.productos, new_row], ignore_index=True)
    ...
    with t3:
        ...
        sa = st.session_state.productos.loc[idx, "Stock"]
        ua = st.session_state.productos.loc[idx, "Unidad"]
        st.info(f"📦 Stock actual: **{sa} {ua}**")
        tm = st.radio("Movimiento", ["➕ Entrada", "➖ Salida"], horizontal=True)
        cant = st.number_input("Cantidad", min_value=1, value=1)
        if st.button("💾 Actualizar Stock"):
            if "Entrada" in tm:
                st.session_state.productos.loc[idx, "Stock"] += cant
                st.success(f"✅ +{cant} {ua}")
            elif cant <= sa:
                st.session_state.productos.loc[idx, "Stock"] -= cant
                st.success(f"✅ -{cant} {ua}")
            else:
                st.error(f"❌ Solo hay {sa} {ua}")
            st.rerun()

# (continúa con las demás funciones: p_pedidos, p_usuarios, p_facturacion, p_reportes, p_prediccion, p_menu, p_mispedidos)
...
def p_pedidos():
    ...
    with t1:
        ...
        filtro = st.selectbox("Filtrar", ["Todos", "Pendiente", "En preparacion", "Entregado"])
        if filtro != "Todos":
            df = df[df["Estado"] == filtro]
        st.dataframe(df.style.applymap(ce, subset=["Estado"]), use_container_width=True, hide_index=True)
        c1, c2, c3 = st.columns([2, 1, 1])
        with c1:
            pid = st.selectbox("Pedido", st.session_state.pedidos["ID"].tolist())
        with c2:
            ne = st.selectbox("Nuevo estado", ["Pendiente", "En preparacion", "Entregado"])
        with c3:
            st.markdown("<br>", unsafe_allow_html=True)
            if st.button("🔄 Actualizar"):
                idx = st.session_state.pedidos[st.session_state.pedidos["ID"] == pid].index[0]
                st.session_state.pedidos.loc[idx, "Estado"] = ne
                st.success(f"✅ {pid} → {ne}")
                st.rerun()
    with t2:
        with st.form("fped"):
            c1, c2 = st.columns(2)
            with c1:
                mesa = st.number_input("Mesa", min_value=1, max_value=20, value=1)
                cliente = st.text_input("Cliente *")
            with c2:
                plato = st.selectbox("Plato", [p[0] for p in PLATOS])
                cant = st.number_input("Cantidad", min_value=1, value=1)
                precio = next(p[3] for p in PLATOS if p[0] == plato)
                st.info(f"💰 Total: **${precio * cant:,} COP**")
            if st.form_submit_button("✅ Registrar Pedido", use_container_width=True):
                if cliente.strip():
                    nid = f"PED-{len(st.session_state.pedidos)+1:03d}"
                    new_row = pd.DataFrame([{"ID":nid, "Mesa":mesa, "Cliente":cliente, "Plato":plato, "Cant":cant, "Total":precio*cant, "Estado":"Pendiente", "Fecha":str(date.today())}])
                    st.session_state.pedidos = pd.concat([st.session_state.pedidos, new_row], ignore_index=True)
                    st.success(f"✅ {nid} registrado")
                    st.rerun()
                else:
...
def p_usuarios():
    ...
    with t1:
        ...
        c1, c2, c3 = st.columns([2, 1, 1])
        with c1:
            us = st.selectbox("Usuario", df["Nombre"].tolist())
        with c2:
            ne = st.selectbox("Estado", ["Activo", "Inactivo"])
        with c3:
            st.markdown("<br>", unsafe_allow_html=True)
            if st.button("💾 Guardar"):
                idx = df[df["Nombre"] == us].index[0]
                st.session_state.usuarios.loc[idx, "Estado"] = ne
                st.success(f"✅ {us} → {ne}")
                st.rerun()
    with t2:
        with st.form("fu"):
            ...
            with c2:
                rol = st.selectbox("Rol", ["Mesero", "Administrador"])
                estado = st.selectbox("Estado", ["Activo", "Inactivo"])
            if st.form_submit_button("✅ Registrar", use_container_width=True):
                if nombre.strip() and correo.strip():
                    nid = f"U{len(st.session_state.usuarios)+1:03d}"
                    new_row = pd.DataFrame([{"ID":nid, "Nombre":nombre, "Correo":correo, "Rol":rol, "Estado":estado}])
                    st.session_state.usuarios = pd.concat([st.session_state.usuarios, new_row], ignore_index=True)
                    st.success(f"✅ {nombre} registrado")
                    st.rerun()
                else:
...
def p_facturacion():
    ...
    with t2:
        ...
        if ent.empty:
            st.info("No hay pedidos entregados para facturar.")
        else:
            with st.form("ff"):
                pid = st.selectbox("Pedido", ent["ID"].tolist())
                ped = ent[ent["ID"] == pid].iloc[0]
                sub = round(ped["Total"] / 1.16)
                iva = ped["Total"] - sub
                ...
                if st.form_submit_button("🧾 Generar Factura", use_container_width=True):
                    ...
                    new_row = pd.DataFrame([{"Factura":f"F-{n:04d}", "Cliente":ped["Cliente"], "Mesa":ped["Mesa"], "Subtotal":sub, "IVA":iva, "Total":ped["Total"], "Fecha":str(date.today()), "Estado":"Pagada"}])
                    st.session_state.facturas = pd.concat([st.session_state.facturas, new_row], ignore_index=True)
                    st.success(f"✅ Factura F-{n:04d} — {metodo} — ${ped['Total']:,} COP")
                    st.rerun()
...
def p_reportes():
    ...
    with t1:
        fechas = pd.date_range(end=date.today(), periods=14).strftime("%d/%m").tolist()
        ventas = [random.randint(60000, 320000) for _ in fechas]
        st.bar_chart(pd.DataFrame({"Ventas (COP)": ventas}, index=fechas), color="#f59e0b")
        c1, c2, c3 = st.columns(3)
        c1.metric("💰 Total", f"${sum(ventas):,.0f}")
        c2.metric("📊 Promedio/día", f"${sum(ventas)//14:,.0f}")
        c3.metric("🏆 Mejor día", f"${max(ventas):,.0f}")
    with t2:
        df = st.session_state.productos
        st.bar_chart(df.groupby("Cat")["Stock"].sum().rename("Stock Total"), color="#f59e0b")
        df2 = df.copy()
        df2["Valor"] = df2["Stock"] * df2["PC"]
...
def p_prediccion():
    ...
    preds = [base + random.randint(-2, 4 + 2*(i >= 4)) for i in range(7)]
    total = sum(preds)
    st.bar_chart(pd.DataFrame({"Demanda estimada": preds}, index=dias), color="#f59e0b")
    c1, c2, c3 = st.columns(3)
    c1.metric("📦 Stock actual", f"{sa} {ua}")
    c2.metric("📊 Demanda semanal", f"{total} {ua}")
    ...
    if deficit > 0:
        st.warning(f"⚠️ Comprar **{deficit} {ua}** de **{ps}** para cubrir la demanda semanal.")
    else:
        st.success(f"✅ Stock de **{ps}** suficiente para la semana.")
    st.markdown("---")
    st.markdown("#### Niveles de riesgo de desabastecimiento")
    df = st.session_state.productos.copy()
    ...
    st.markdown("---")
    st.markdown("#### 🍽️ Platos con mayor demanda estimada")
    st.dataframe(pd.DataFrame({
        "Plato": ["Bandeja paisa", "Sancocho de gallina", "Cazuela de mariscos", "Arroz con pollo"],
        "Pedidos/semana": [45, 38, 34, 29],
        "Tendencia": ["📈 Subiendo", "➡️ Estable", "📈 Subiendo", "📈 Subiendo"]
    }), use_container_width=True, hide_index=True)

def p_menu():
    hdr("📋 Menú del Restaurante", "Platos disponibles hoy")
    cats = list({p[2] for p in PLATOS})
    cat_sel = st.selectbox("Categoría", ["Todas"] + cats)
    filtrado = [p for p in PLATOS if cat_sel == "Todas" or p[2] == cat_sel]
    cols = st.columns(2)
...
def p_mispedidos():
    ...
    with st.form("fcli"):
        ...
        mesa = st.number_input("N° de mesa", min_value=1, max_value=20, value=1)
        plato = st.selectbox("¿Qué deseas ordenar?", [p[0] for p in PLATOS])
        precio = next(p[3] for p in PLATOS if p[0] == plato)
        cant = st.number_input("Cantidad", min_value=1, value=1)
        st.info(f"💰 Total: **${precio * cant:,} COP**")
        ...
        if st.form_submit_button("📲 Enviar Pedido", use_container_width=True):
            if nombre.strip():
                st.success(f"✅ ¡Pedido enviado! Mesa {mesa} — **{plato}** x{cant} 🍽️")
            else:
